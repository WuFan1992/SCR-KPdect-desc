...
class GeneralHead(nn.Module):
    ...
    def forward(self, feat, corr, s_coords_grid, p_coords):
        ...
        pred_coords2, x = self.res_head(x) # 只是用top k source coordinate 和  query source 的相似度， 对坐标的residual 进行预测，这里没有使用CNN 提取的feature

        if p_coords is not None:
            pred_coords2 = pred_coords2 + p_coords # 接着这个residual 用来更新坐标
            x = torch.cat([p_coords, x], dim=1)

        pred_coords1 = self.dense_head(feat, x) #使用了CNN 提取的feature, 和 用来对residual 预测的feature 融合，利用语义信息进行预测

        return pred_coords1, pred_coords2, None # 输出针对同一个query 不同方式的坐标预测。前者针对与source 的关系，基于匹配预测，精度高但是如果没有足够的与source 关系，可能会失败
                                                # 后者针对语义信息进行预测，但可能会面临精度上的问题
