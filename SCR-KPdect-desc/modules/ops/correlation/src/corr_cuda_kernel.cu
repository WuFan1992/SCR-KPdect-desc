    ...
    cudaStream_t stream)
{
    ...
    int height  = in.size(2);
    int width   = in.size(3);
    int widthheight = width * height;
    int pwidthheight= widthheight; // 如果需要可以计算
    ...
    blob_rearrange_kernel2<<<totalBlocksRearr, threads_per_block, 0, stream>>>(
        in_ptr, out_ptr, num, channels, width, height, widthheight, padding, pwidthheight);
...
{
    // 从 Tensor 获取尺寸
    int batchSize     = rbot1.size(0);
    int nInputPlane   = rbot1.size(1);
    int paddedbottomheight = rbot1.size(2);
    int paddedbottomwidth  = rbot1.size(3);

    int nOutputCols   = output.size(3);
    int nOutputRows   = output.size(2);
    int nOutputPlane  = output.size(1);
    ...
    const float* rbot1_ptr = rbot1.data_ptr<float>();
    const float* rbot2_ptr = rbot2.data_ptr<float>();
    ...
    if (corr_type_multiply == 1) {
        ...
        CorrelateData<<<totalBlocksCorr, threadsPerBlock, shared_memory_per_block * sizeof(float), stream>>>(
            ...
            rbot1_ptr, rbot2_ptr, output_ptr
        );
    } else {
    ...
    cudaStream_t stream)
{
    int batchSize     = rbot1.size(0);
    int nInputPlane   = rbot1.size(1);
    int nInputRows    = rbot1.size(2);
    int nInputCols    = rbot1.size(3);
    int paddedbottomheight = nInputRows;
    int paddedbottomwidth  = nInputCols;

    int nOutputCols   = gradOutput.size(3);
    int nOutputRows   = gradOutput.size(2);
    ...
    const float* rbot1_ptr     = rbot1.data_ptr<float>();
    const float* rbot2_ptr     = rbot2.data_ptr<float>();
    const float* gradOutput_ptr= gradOutput.data_ptr<float>();
    float* gradInput1_ptr      = gradInput1.data_ptr<float>();
    float* gradInput2_ptr      = gradInput2.data_ptr<float>();
    ...
    if (corr_type_multiply == 1) {
        ...
        for (int n = 0; n < batchSize; n++) {
            CorrelateDataBackward0<<<GET_BLOCKS(botThreadCount, CUDA_NUM_THREADS), CUDA_NUM_THREADS, 0, stream>>>(
                ...
                gradInput1_ptr, rbot2_ptr, gradOutput_ptr
            );
        }
        ...
        for (int n = 0; n < batchSize; n++) {
            CorrelateDataBackward1<<<GET_BLOCKS(botThreadCount, CUDA_NUM_THREADS), CUDA_NUM_THREADS, 0, stream>>>(
                ...
                kernel_radius_, stride1, stride2, nInputCols, nInputRows,
                paddedbottomwidth, paddedbottomheight, nInputPlane, inputCount, pad_size,
                rbot1_ptr, gradInput2_ptr, gradOutput_ptr
            );
        }
        ...
        for (int n = 0; n < batchSize; n++) {
            CorrelateDataBackward0Subtract<<<GET_BLOCKS(botThreadCount, CUDA_NUM_THREADS), CUDA_NUM_THREADS, 0, stream>>>(
                ...
                kernel_radius_, stride1, stride2, nInputCols, nInputRows,
                paddedbottomwidth, paddedbottomheight, nInputPlane, inputCount, pad_size,
                gradInput1_ptr, rbot1_ptr, rbot2_ptr, gradOutput_ptr
            );
        }
        ...
        for (int n = 0; n < batchSize; n++) {
            CorrelateDataBackward1Subtract<<<GET_BLOCKS(botThreadCount, CUDA_NUM_THREADS), CUDA_NUM_THREADS, 0, stream>>>(
                ...
                kernel_radius_, stride1, stride2, nInputCols, nInputRows,
                paddedbottomwidth, paddedbottomheight, nInputPlane, inputCount, pad_size,
                rbot1_ptr, rbot2_ptr, gradInput2_ptr, gradOutput_ptr
            );
        }
    }
