...
__global__ void nms_kernel(
  torch::PackedTensorAccessor32<scalar_t,5,torch::RestrictPtrTraits> coords_2d_grid,
  torch::PackedTensorAccessor32<int64_t,4,torch::RestrictPtrTraits> idxs, // <- 改成 int64_t
  int topk,
  int num_grid,
        ...
        for(int j=0;j<i;j++){
            if(int(round(coords_2d_grid[item][y1][x1][j][1]))==x2_round &&
               ...
               (coords_2d_grid[item][y1][x1][j][1]!=x2 ||
                coords_2d_grid[item][y1][x1][j][0]!=y2)){
                flag=false;
            }
        }
        if(flag){
            idxs[item][idx_cnt][y1][x1] = i; // <- idxs 是 int64_t，没问题
            idx_cnt++;
        }
    }
}
...
std::vector<torch::Tensor> nms_on_gpu(torch::Tensor coords_2d_grid, // N,H,W,M,2
    int topk,
    ...
    // idxs 用 long 类型
    auto idxs = torch::full({N, topk, H, W}, -1, torch::dtype(torch::kInt64).device(torch::kCUDA));
    ...
    AT_DISPATCH_FLOATING_TYPES(coords_2d_grid.scalar_type(), "nms_kernel", ([&] {
        nms_kernel<scalar_t><<<totalBlocksCorr, threadsPerBlock, shared_memory_per_block * sizeof(float)>>>(
            ...
            idxs.packed_accessor32<int64_t,4,torch::RestrictPtrTraits>(), // <- 注意这里是 int64_t
            topk,
            num_grid,
