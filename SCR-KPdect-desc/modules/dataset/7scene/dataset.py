...
class VideoDataset(Dataset):
    def __init__(
        ...
        (
            ...
            self.start_idxs,
            self.end_idxs,
        ) = self.load_and_fuse_datasets("../../../", "datasets/head/deep_retrieve_full.bin")
    ...
    def reset_valid_list(self):
        ...
        for i, meta_data in enumerate(self.meta_info_list):
            if (
                ...
                continue
            topk_list = meta_data["topk"]   # 这里meta_data["topk"] 存储的不是reference 的index ，而是topK 的frame 的结构
                                            # 好处是不需要索引系统，可以直接加载数据，同时实现跨数据集的统一
            new_topk_list = []
            max_overlap = 0
            for j, meta_data_topk in enumerate(topk_list):
                ...
                if self.overlap_check:
                    if (
                        ...
                        new_topk_list.append(meta_data_topk)  # 筛选reference frame
                        max_overlap = max(max_overlap, meta_data["overlap"][j])

                    if max_overlap > self.max_overlap_thresh:
                        meta_data["topk"] = new_topk_list  # 如果至少有一帧有高的重合，则保留这些reference 
                    else:
                        meta_data["topk"] = []  # 如果所有的reference 都是弱的重叠，则废掉这些reference

            if len(meta_data["topk"]) >= self.check_topk:
                valid_idx_list.append(i)  # 只有当前query frame 有足够有效的reference frame 的时候才认为这是一个可用于训练的样本

        return valid_idx_list
    
    # 加载多个训练的数据集，融合在一起形成完整的一个数据集。
  ...
  12,12,12,12,12  # seq2
    ...
    # 把原始的meta 信息转变成frame 的列表，即datasets = [[frame1, frame2, frame3..]   // seq 1
    # , [frame1, frame2, frame3...]   // seq2]
    def load_general_datasets(self, base_dir, seqs):
        # seqs 可能有两种可能性 
        # 一种是list 结构 seqs = [frame_dict1, frame_dict2, frame_dict3 ...]
        # 另一种是 dict 结构 seqs = {"scene0001": [frame1, frame2], "scene0002": [frame1, frame2]}  SCANET 是这种情况
        ...
        if type(seqs) is list:
            ...
            for frame in seqs:
                ...
                new_seq.append(frame) # 给每一个frame 都添加路径
        ...
        return datasets     # 原来数据是按照seq 来组成的字典形式，现在同一放入一个list 里面 便于遍历
        
    """
    frame 的格式如下
    
    frame = {
    "file_name": "scene0001/color/000123.jpg",
    "depth_name": "scene0001/depth/000123.png",
    "pose_name": "scene0001/pose/000123.txt",
    "K": ...,
    "Tcw": ...,
    "overlap": [...],
    "topk": [...],
    ...
    def load_seq_scene(self, topk_meta_info_list, base_dir, q_Tcw, q_K, q_depth):
        ...
        Tcw_tensor = np.stack(Tcw_tensor).astype(np.float32)  # 将list 变成 nadrray
        K_tensor = np.stack(K_tensor).astype(np.float32)
        depth_tensor = np.stack(depth_tensor).astype(np.float32)
        ...
        result = {
            ...
            "K": K_tensor,
            "depth": depth_tensor,
            "img": np.stack(img_tensor).astype(np.float32).transpose(0, 3, 1, 2),  # (N L 3 H W) 其中L= ref_topk 如果topk_meta_info_list
            #的个数小于ref topk，则自动补齐到ref_topk; 如果大于，则自动截断。
        } 

        return result, topk_meta_info_list
    
    # 给模型构造一个训练样本= 一个query 序列+ 多视角的source 的图像结合
    def __getitem__(self, idx):
        p = 0
        q_result, r_result = None, None
        idx = self.valid_idx_list[idx]
        # idx=1847
        while r_result is None:  #当找到合适的referenece 就推出，否则就继续执行直到找到一个合适的
            ...
            _idx = idx
            idxs = []
            random_temporal_len = self.tempo_len  # 一次只提取一个query image
            ...
            q_result["img"]   : (T, 3, H, W)
            q_result["depth"] : (T, H, W)
            q_result["Tcw"]   : (T, 3, 4)
            q_result["K"]     : (T, 3, 3)
