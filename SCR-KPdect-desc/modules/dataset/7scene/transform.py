    ...
    def __call__(self, img, depth, Tcw, K):
        ...
        if img is not None:
            ...
            if h <= w:
                ...
                img = img.resize((self.h, self.w), resample=Image.Resampling.LANCZOS)
            img = np.asarray(img)

        depth = Image.fromarray(depth)
        if h <= w:
            depth = depth.resize((self.w, self.h), resample=Image.Resampling.NEAREST)
        else:
            depth = depth.resize((self.h, self.w), resample=Image.Resampling.NEAREST)
