...
from modules.utils.scene_utils import crop_by_intrinsic
...
class BaseDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        pair_data = {
            ...
            'image0_ori': np.array(res_image0["img_ori"][0], dtype=np.float32),    # (C, H, W)
            'depth0': np.array(res_image0["depth"][0], dtype=np.float32),  # (H, W)
            'image1': np.array(res_image1["img"][0], dtype=np.float32),
            'image1_ori': np.array(res_image1["img_ori"][0], dtype=np.float32),
...
class SevenSceneDataset(BaseDataset):
    def __init__(self,
                 ...
                 mode='train',
                 augment_fn=None):
        super().__init__(cfg, transform, normalize,mode, augment_fn)
