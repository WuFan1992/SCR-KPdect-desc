...
class DSMNet(nn.Module):
    ...
    ### 对同一个点在不同视角下得到的特征进行融合 + 用粗尺度的信息去填补细尺度中由于几何不可见而
    ### 造成的空洞
    def fuse_and_fill_holes_pyramid(
        ...
        ] # 多视角feature 融合 输出 一个包含多个N*L*C*H*W的list ，每一个元素代表一个scale

        N, L = feat_pyramid[0].shape[:2] 
        f_up, c_up = flattenNL(feat_pyramid[0]), flattenNL(coords_pyramid[0]) # 将feature map 展平成 f_up : (N*L, C, H0, W0) c_up:(N*L, 3, H0,W0) 
        new_feat_pyramid = [feat_pyramid[0]]
        new_coords_pyramid = [coords_pyramid[0]]
        new_mask_pyramid = [mask_pyramid[0]]    # 保存最coarse 的部分，因为这一scale 下不需要fill

        for f, c, m in zip(feat_pyramid[1:], coords_pyramid[1:], mask_pyramid[1:]):
            f = flattenNL(f)
            c = flattenNL(c)
            m = flattenNL(m)
            f, c = self.fill_holes(f, c, m, f_up, c_up, fill_feat=False)   # 如果当前尺寸的某些像素没有有效几何(mask=0),就从上一个
                                                                           # 更粗的尺度中，用几何一致的方式补回来
            ...
            f_up = f
            c_up = c
            #new_feat_pyramid   : List[ Tensor (N, L, C, H_l, W_l) ]
            #new_coords_pyramid : List[ Tensor (N, L, 3, H_l, W_l) ]
            #new_mask_pyramid   : List[ Tensor (N, L, H_l, W_l) ]  list 里面的元素个数是pyramid 的层数
    ...
    ## q_feat_pyramid: [
  #(N, T, C₀, H₀, W₀),   # finest / highest resolution
  #(N, T, C₁, H₁, W₁),
  #(N, T, C₂, H₂, W₂),
  #...
  #]
    ...
    # 对于每一个query 像素，从所有source 图像的所有像素中，找出与之最相似的一批source 特征
    # 并取出这些source 像素对应的3D 坐标作为候选
    # 两种模式， p_coords is None. 全局dense matching，p_coords != None 在投影领域内refinement
    
    def retrieve_topk(
        self, q_feat_norm, s_feat_norm, s_coords, s_P, r_P, p_coords=None
    ):
        N, L, C, H, W = s_feat_norm.shape
        if p_coords is None: # 最粗层，全局dense matching 
            q_feat_norm = q_feat_norm.reshape(N, C, -1) # (N,C,HW) 为计算相似度的矩阵乘法做准备
            s_feat_norm = s_feat_norm.permute(0, 1, 3, 4, 2).reshape(N, -1, C) # (N,LHW,C)  为计算相似度的矩阵乘法做准备
            corr = torch.bmm(s_feat_norm, q_feat_norm).reshape(N, -1, H, W) # (N,LHM,H,W) 第2维是所有source 像素的总数，第3 4维是query 图像空间位置
            s_coords_grid = (
                s_coords.permute(0, 1, 3, 4, 2)
                .reshape(N, -1, 3, 1, 1)
                .repeat(1, 1, 1, H, W)
            ) # (N, L * H * W, 3, H, W) 将每一个source 像素，取它的3D坐标，然后复制到query 像素位置。作为候选
            # 这是因为query 里面的每一个像素，它都需要和所有source 里的所有像素都计算一次相似度。
            ...
            ) # 用上一层预测的坐标，将query 里面的每一个像素，利用S_P 投影到每一个source view, 然后在source view 里面取一个小窗口，在这个窗口内
            # 计算correlation 

        return self.sort_corr(corr, s_coords_grid, mask, r_P) #corr : (N, K, H, W)  s_coords_grid  : (N, K, 3, H, W) mask : (N, K, H, W)
    
    ## 每一个source view 作为reference ,然后把其他source view的特征通过3D scene coordinate 
    ## 对齐到它的像素平面上。然后在对齐后的view 维度做平均融合。就可以得到多视角一致的source 特征
    ## 输入是 feat  (N L C H W) 代表L 个source 的特征图
    ##        s_coords: (N,L,3,H,W)  每一个source 像素对应的3D 点坐标 (在reference frame 参考系之下)
    ##        s_P: (N, L, 3, 4)  每一个source 的投影矩阵
    def fuse_feat(self, feat, s_coords, s_P, self_mask=True):

        N, L, C, H, W = feat.shape
        scene_feat_list = [] # 保存每一个source 作为reference 后融合的结果 一共有L 个结果
        mask_list = []
        for l in range(s_coords.shape[1]):
            x_2d, m = projection(s_coords[:, l, :, :, :], s_P) # x……2d (N,L,2,H,W) 投影后的2D 坐标   m (N,L,H,W) 投影是否有效 
            ...
            x_2d = self.normalize_2d_coords(x_2d, dim=3) # x_2d (N*L,H,W,2) 将2D 坐标转换到grid sample 所需要的格式

            scene_align_feat = F.grid_sample(
                feat.reshape(N * L, -1, H, W), x_2d, mode="bilinear", align_corners=True
            ) # 不同的L代表不同的source 也就是不同的视角，将每一个source 都作为一次reference ,然后根据投影矩阵投影到其他source 上，然后提取
            #投影点的位置的特征，就得到同一个3D点在不同视角下的特征
            scene_align_feat = scene_align_feat.reshape(N, L, -1, H, W) # (N, L, C, H, W)

            scene_align_feat = scene_align_feat * m.unsqueeze(2) # 不可投影或者被遮挡的点，不参与融合

            scene_feat_avg = scene_align_feat.sum(dim=1) / (
                m.sum(dim=1, keepdim=True) + 1e-7
            ) # 对于同一reference 里面的像素对应的3D 点，在其他source 下投影位置的特征做平均

            scene_feat_list.append(scene_feat_avg.unsqueeze(1)) # 每一张source 都能产生一张多视角融合后的特征图
            mask_list.append(m)
    ...
    # 利用 query source 的特征相关性，从source 的scene coordinates 中检索候选3D点，
    # 然后在coarse to fine 约束下进行局部refinement 
    # 最后回归query像素的3D scene coordinate 同时输出不确定性map
    def predict_coords(
        self,
        q_feat_combined, # (N, T, C, H, W)
        s_feat,   # (N,L,C,H,W)
        s_coords,  # (N,L,3,H,W)
        q_P,    # (N, T, 3, 4) query 投影矩阵 
        s_P,    # (N, L, 3, 4) source 投影矩阵
        r_P,    # (N, 3, 4)  reference 投影矩阵
        head,   # 当前尺度的回归头
        p_coords=None,  # 上一尺度的预测
        q_K=None,
        conf_head=None,
        scores=None,
    ):
        ...
        ) # 找到query 里面每一个像素 top-k 最相似的 source 像素
        ...
        pred_coords1, pred_coords2, scores = head(q_feat, corr, s_coords_grid, p_coords) # 预测坐标
        ...
        return pred_coords1, pred_coords2, scores, s_coords_grid_ori, std  # s_coords_grid_ori指的是source 里面的top k 的坐标 

    # 在多尺度 coarse to fine 下，利用query source 的特征相关性，逐层预测query 图像中每一个像素对应的3D scene coordinate
    def predict_coords_pyramid(
        self,
        q_feat_pyramid,  # q_feat_pyramid[i] : (N, T, C, H_i, W_i) query 的特征 
        s_feat_pyramid,  # s_feat_pyramid[i]: (N, L, C, H_i, W_i) source 的特征
        s_coords_pyramid, # s_coords_pyramid : (N, L, 3, H_i, W_i) source 每个点对应的3D 坐标
        q_P_pyramid,      # q_P_pyramid: (N, T, 3, 4)  query 的投影矩阵
        s_P_pyramid,      # s_P_pyramid: (N, L, 3, 4)  source 的投影矩阵
        r_P_pyramid,      # r_P_pyramid: (N, 3, 4)    reference 的投影矩阵
        q_K_pyramid=None,
        conf=None,
    ):
    ...
    def forward(
        ...
        (
            s_feat_pyramid, # List[ Tensor (N, L, C, H_l, W_l) ]
            s_coords_pyramid,  #  List[ Tensor (N, L, 3, H_l, W_l) ]
            s_mask_pyramid,    # List[ Tensor (N, L, H_l, W_l) ]
        ...
        ).reshape(N, -1, 3, H, W)  # 将source GT里面的每一个像素投影到世界坐标系下得到3D坐标
        q_coords = back_projection(
            q_depth[:, 0, :, :], q_Tcw[:, 0, :, :], q_K[:, 0, :, :]
        )  # 将query GT 投影到世界坐标系下的3D 坐标，只用第一张图

        gt_coords_pyramid = [q[:, 0, :, :, :].contiguous() for q in q_coords_pyramid] # 提取pyramid 级别的GT 
        gt_mask_pyramid = [m[:, 0, :, :].contiguous() for m in q_mask_pyramid]

        q_P_pyramid = [q_P[:, 0, :, :] for q_P in q_P_pyramid] # 当前
        ...
        pred_coords = self.rot_coords(r_Tcw, pred_coords1_pyramid[-1]) # 把scene coordinates 从reference camera 转换成世界坐标系下的坐标
